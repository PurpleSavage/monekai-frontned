...
@Component({
  selector: 'app-latest-shared-samples-page',
  templateUrl: './latest-shared-samples-page.component.html',
  standalone: true,
  imports: [ListLatestSharedSamplesVersionComponent, ListLatestEditSharedSamplesComponent]
})
export class LatestSharedSamplesPageComponent implements OnInit {
    ...
    this.listSamplesEditedUseCase.execute(dto).subscribe({
      ...
      error: (error) => {
        if (error instanceof AppBaseError) {
          this.errorLisEditSharedSamples.set(error.message);
        }
      }
    });
  }
  ...
  listSharedSamples(dto: LatestRequestDTO): void {
    this.listSahredSamplesUseCase.execute(dto).subscribe({
      ...
      error: (error) => {
        if (error instanceof AppBaseError) {
          this.errorLisSharedSamples.set(error.message);
        }
      }
    });
  }
